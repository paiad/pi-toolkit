...
| [pi-cc-extensions](https://pi.dev/packages/pi-cc-extensions)（上游 [minuque/pi-cc-extensions](https://github.com/minuque/pi-cc-extensions)，MIT） | 在所有项目中提供 Claude Code 风格 TUI：工具摘要、折叠卡片、rich edit/write diff、`/ccstyle` 面板、`/context` 上下文检查、`@` session/subagent 引用、CC Dark/Light 主题；可配置项都在 `~/.pi/agent/claude-code-style.json` |
