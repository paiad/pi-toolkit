...
import type { RainbowPalette } from "./config.ts";
...
/** Per-character rainbow gradient (24-bit truecolor) over any label, e.g. a model name. */
export function rainbowText(text: string, palette: RainbowPalette): string {
	const chars = Array.from(text);
	const last = Math.max(1, chars.length - 1);
	const span = Math.min(palette.hueSpan, last * palette.huePerChar);
	return chars
		.map((char, i) => {
			if (char === " ") return char;
			const [r, g, b] = hslToRgb(palette.hueStart + (i / last) * span, palette.saturation, palette.lightness);
