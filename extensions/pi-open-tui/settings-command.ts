...
import type { CursorStyle, IconMode, ModelNameStyle, OpenTuiConfig, SettingsLanguage, ThinkingPeekLines } from "./config.ts";
import {
...
const COPY = {
	en: {
		...
		values: {
			...
			modelNameStyles: { plain: "Plain", rainbow: "Rainbow" },
		},
	},
	zh: {
		title: "Open TUI 设置",
		...
		values: {
			...
			modelNameStyles: { plain: "纯色", rainbow: "彩虹" },
		},
	},
} as const;
...
function cycleModelNameStyle(config: OpenTuiConfig): OpenTuiConfig {
	const next: ModelNameStyle = config.modelName.style === "rainbow" ? "plain" : "rainbow";
	return { ...config, modelName: { ...config.modelName, style: next } };
}
...
function buildIconsItems(config: OpenTuiConfig, copy: SettingsCopy): SettingItem[] {
	return [
		...
		{ id: "modelNameStyle", label: copy.labels.modelName, currentValue: copy.values.modelNameStyles[config.modelName.style] },
	];
}
...
function handleSettingChange(
	...
	if (tab === "icons") {
		...
		if (itemId === "modelNameStyle") return cycleModelNameStyle(config);
	}
	if (tab === "segments") {
