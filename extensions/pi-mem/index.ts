 ...
 *
 * Layout (under <project>/.pi/memory/ by default):
...
export function resolveMemoryDir(env: MemoryEnv = process.env, projectDir = process.cwd()): string {
	if (env.PI_MEMORY_DIR) return env.PI_MEMORY_DIR;
	return path.join(projectDir, ".pi", "memory");
}
...
/**
 * qmd collection names are global to the user's qmd installation. Include a
 * stable project-path hash so two Pi projects never search one another's memory.
 */
export function getQmdCollectionName(memoryDir = MEMORY_DIR): string {
	const scopeHash = createHash("sha256").update(path.resolve(memoryDir)).digest("hex").slice(0, 12);
	return `pi-mem-${scopeHash}`;
}
...
export function qmdInstallInstructions(): string {
	return [
		...
		`  qmd collection add ${MEMORY_DIR} --name ${getQmdCollectionName()}`,
		"  qmd embed",
	].join("\n");
}
...
export function qmdCollectionInstructions(): string {
	return [
		`qmd collection ${getQmdCollectionName()} is not configured.`,
		"",
...
export async function setupQmdCollection(): Promise<boolean> {
	try {
		await new Promise<void>((resolve, reject) => {
			execFileFn("qmd", ["collection", "add", MEMORY_DIR, "--name", getQmdCollectionName()], { timeout: 10_000 }, (err) =>
	...
	for (const [ctxPath, desc] of contexts) {
		try {
			await new Promise<void>((resolve, reject) => {
				execFileFn("qmd", ["context", "add", ctxPath, desc, "-c", getQmdCollectionName()], { timeout: 10_000 }, (err) =>
					err ? reject(err) : resolve(),
				);
			});
		} catch {
	...
	// Seed the cache so checkCollection(getQmdCollectionName()) doesn't redundantly re-run
	// setupQmdCollection during the short negative-cache window.
	qmdCollectionStatusCache.set(getQmdCollectionName(), { checkedAt: Date.now(), exists: true });
	return true;
}
...
export async function searchRelevantMemories(prompt: string): Promise<string> {
	...
	try {
		const hasCollection = await checkCollection(getQmdCollectionName());
...
export function runQmdSearch(
	...
	const args = [subcommand, "--json", "-c", getQmdCollectionName(), "-n", String(limit), query];
...
export default function (pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
			...
			let hasCollection = await checkCollection(getQmdCollectionName());
			if (!hasCollection) {
	...
	pi.registerTool({
		...
		async execute(_toolCallId, _params, _signal, _onUpdate, _ctx) {
			...
			if (qmdOk) {
				collectionOk = await checkCollection(getQmdCollectionName());
			...
			if (qmdOk) {
				lines.push(`- Collection \`${getQmdCollectionName()}\`: ${mark(collectionOk)}`);
				if (collectionOk) {
